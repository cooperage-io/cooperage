...
Exposes a single MCP server to the LLM. Tools:
  ...
  cooperage_list_tools        — list tools exposed by a given server
...
import cooperage.orchestrator.docker as orch
from cooperage.core.models import ContainerInfo
...
async def _dispatch(name: str, args: dict[str, Any]) -> Any:
    ...
    if name == "cooperage_create_session":
        return _create_session(args.get("name"))
...
async def _ensure_container(session_id: str, server_name: str) -> ContainerInfo:
    ...
    if server_def is None:
        raise ValueError(f"No server named {server_name!r} in registry")
    return sessions.get_or_start_container(session_id, server_def)
